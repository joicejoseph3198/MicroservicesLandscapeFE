...
export const ProductListing = () => {
  ...
  return (
    <div className="w-full h-screen flex flex-col pl-12 overflow-scroll">
      <div className="h-screen bg-white z-8">
        <p className=" sticky text-5xl lg:text-8xl text-left text-slate-700 font-['NeueBit'] py-4">Dashboard/Product Listing</p>
        {/* BUTTON ROW --START*/}
        <div className=" min-w-[1000px] grid grid-cols-5 gap-4 py-4 px-5">
        ...
        <div className={`w-[${table.getTotalSize()}] rounded-md p-2 flex-1 no-wrap justify-around w-full border-2 text-md`}>
          {/* HEADER --START */}
          <div className={`w-[${table.getTotalSize()}] thead pb-1`}>
            {table.getHeaderGroups().map((headerGroup) => (
              <div
                key={headerGroup.id}
                className="flex flex-row no-wrap justify-between w-full tr text-sm text-slate-700 font-['MoriBold']"
              >
          ...
          <div>
            {table.getRowModel().rows.map((row) => (
              <div key={row.id} className="flex flex-row items-center text-sm justify-around hover:bg-blue-500 hover:text-white hover:hover:cursor-pointer"
